...
# dictionary to hold the varuious light types used by ogre and the 
# equivelents in the blender export
...
def getNode(base, name):
    ...
    existance and a getter for that element node
    """
    ...
    def parseDotScene(self):
        ...
        for node in self.docRoot:
            if node.nodeType == Node.ELEMENT_NODE and node.nodeName == 'node':
                ...
                qnode = getNode(node, 'quaternion')
                if qnode:
                  rotationNode = node.attributes
                ...
                snode = getNode(node, 'scale')
                if snode:
                  scaleNode = node.attributes
